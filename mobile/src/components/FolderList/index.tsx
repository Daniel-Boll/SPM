...
const ConfirmableButton = (props) => {
  ...
  const handlePress = () => {
    confirm({
      theme: styles.confirmDialog,
      onConfirm: onPress,
      cancelButtonStyle: styles.cancelDialog,
      cancelButtonLabelStyle: styles.cancelDialogLabel,
    });
  };
...
const ListItemRender = (props: IListItem) => {
  const {
    item: { name, description, onClickEdit, onClickDelete, onClickNavigate },
  } = props;
  return (
    <ListItem
      style={styles.item}
      ...
      accessoryLeft={() => (
        <Button onPress={onClickEdit} status="primary" accessoryLeft={EditIcon} />
      )}
      accessoryRight={() => (
        <ConfirmableButton
          onPress={() => onClickDelete(name)}
          status="danger"
